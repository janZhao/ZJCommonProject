...
-(void)doneClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:indexArray:)]) {
        NSMutableArray *indexArray = nil;
        if (_pickerView) {
            indexArray = [NSMutableArray array];
            for (int i = 0; i < self.pickerView.numberOfComponents; i++) {
                [indexArray addObject:[NSNumber numberWithInteger:[self.pickerView selectedRowInComponent:i]]];
            }
        }
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString indexArray:indexArray];
    }
